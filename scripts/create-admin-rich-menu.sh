#!/usr/bin/env bash
# 院長用リッチメニューを作成し、画像をアップロードするスクリプト
#
# 使い方:
#   KEY=lh_xxx LIFF_URL_ADMIN=https://liff.line.me/xxxx bash scripts/create-admin-rich-menu.sh ./admin-menu.png <line_account_id>
#
# 環境変数:
#   API           — Worker の URL（デフォルト: https://line-harness.nogardwons.workers.dev）
#   KEY           — API キー（必須）
#   LIFF_URL_ADMIN — 院長向けLIFF URL（デフォルト: https://line-harness-web-a61.pages.dev/booking/list）

set -euo pipefail

# ---------- 引数チェック ----------
IMAGE_FILE="${1:-}"
LINE_ACCOUNT_ID="${2:-}"

if [[ -z "$IMAGE_FILE" || -z "$LINE_ACCOUNT_ID" ]]; then
  echo "Usage: KEY=lh_xxx bash $0 <image_file> <line_account_id>" >&2
  exit 1
fi

if [[ ! -f "$IMAGE_FILE" ]]; then
  echo "Error: image file not found: $IMAGE_FILE" >&2
  exit 1
fi

# ---------- jq チェック ----------
if ! command -v jq &>/dev/null; then
  echo "Error: 'jq' is required but not installed." >&2
  echo "  macOS: brew install jq" >&2
  echo "  Linux: sudo apt-get install jq" >&2
  exit 1
fi

# ---------- 環境変数チェック ----------
API="${API:-https://line-harness.nogardwons.workers.dev}"
LIFF_URL_ADMIN="${LIFF_URL_ADMIN:-https://line-harness-web-a61.pages.dev/booking/list}"

if [[ -z "${KEY:-}" ]]; then
  echo "Error: KEY environment variable is required." >&2
  exit 1
fi

# ---------- ステップ1: リッチメニュー構造を登録 ----------
echo "Creating rich menu structure..."

RICH_MENU_PAYLOAD='{
  "size": { "width": 2500, "height": 843 },
  "selected": true,
  "name": "院長メニュー",
  "chatBarText": "メニュー",
  "areas": [
    {
      "bounds": { "x": 0, "y": 0, "width": 833, "height": 843 },
      "action": { "type": "postback", "label": "今日の予約", "data": "action=admin_today_bookings" }
    },
    {
      "bounds": { "x": 833, "y": 0, "width": 833, "height": 843 },
      "action": {
        "type": "uri",
        "label": "店舗設定",
        "uri": "${LIFF_URL_ADMIN}"
      }
    },
    {
      "bounds": { "x": 1666, "y": 0, "width": 834, "height": 843 },
      "action": { "type": "postback", "label": "明日の予約", "data": "action=admin_tomorrow_bookings" }
    }
  ]
}'

CREATE_RESPONSE=$(curl -s -X POST \
  "${API}/api/rich-menus?line_account_id=${LINE_ACCOUNT_ID}" \
  -H "Authorization: Bearer ${KEY}" \
  -H "Content-Type: application/json" \
  -d "$RICH_MENU_PAYLOAD")

CREATE_SUCCESS=$(echo "$CREATE_RESPONSE" | jq -r '.success')
if [[ "$CREATE_SUCCESS" != "true" ]]; then
  echo "Error: Failed to create rich menu." >&2
  echo "$CREATE_RESPONSE" | jq . >&2
  exit 1
fi

RICH_MENU_ID=$(echo "$CREATE_RESPONSE" | jq -r '.data.richMenuId')
if [[ -z "$RICH_MENU_ID" || "$RICH_MENU_ID" == "null" ]]; then
  echo "Error: richMenuId not found in response." >&2
  echo "$CREATE_RESPONSE" | jq . >&2
  exit 1
fi

echo "Rich menu created: $RICH_MENU_ID"

# ---------- ステップ2: 画像をアップロード ----------
echo "Uploading image: $IMAGE_FILE"

UPLOAD_RESPONSE=$(curl -s -X POST \
  "${API}/api/rich-menus/${RICH_MENU_ID}/image?line_account_id=${LINE_ACCOUNT_ID}" \
  -H "Authorization: Bearer ${KEY}" \
  -H "Content-Type: image/png" \
  --data-binary "@${IMAGE_FILE}")

UPLOAD_SUCCESS=$(echo "$UPLOAD_RESPONSE" | jq -r '.success')
if [[ "$UPLOAD_SUCCESS" != "true" ]]; then
  echo "Error: Failed to upload image." >&2
  echo "$UPLOAD_RESPONSE" | jq . >&2
  exit 1
fi

echo "Image uploaded successfully."

# ---------- 完了: richMenuId を標準出力 ----------
echo ""
echo "=========================================="
echo "richMenuId: $RICH_MENU_ID"
echo "=========================================="
echo ""
echo "次のステップ: 以下の SQL で admin_rich_menu_id を更新してください。"
echo ""
echo "  npx wrangler d1 execute line-harness --remote --command \\"
echo "    \"UPDATE line_accounts SET admin_rich_menu_id = '${RICH_MENU_ID}' WHERE id = '${LINE_ACCOUNT_ID}';\""
